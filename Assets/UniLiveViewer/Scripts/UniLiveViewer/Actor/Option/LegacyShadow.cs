...
{
    public class LegacyShadow : IFakeShadow
    {
        const string PrefabPath = "Prefabs/Shadow/LegacyShadow";
        ...
        readonly List<(Transform parentBone, MeshRenderer shadow)> _map = new();
        ...
        public void Setup(Transform parent, SHADOWTYPE shadowType, QuasiShadowSetting.Preset preset, float userShadowScale)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                var shadow = go.GetComponent<MeshRenderer>();
        ...
        void Transforming(Transform targetBone, MeshRenderer shadow, float settingsScala, float attenuationMultiplier)
        {
